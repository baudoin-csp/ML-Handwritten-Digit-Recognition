...
from src.utils import normalize_fn, append_bias_term, accuracy_fn, macrof1_fn, get_n_classes, get_time_analyse
...
def main(args):
    ...
    if not args.test:
        if args.method == "nn":
            xtrain = normalize_fn(train_data, means, stds)
            ytrain = train_labels
            xtest = normalize_fn(validation_data, means, stds)
    ...
    if args.use_pca:
        ...
        # Find the principal components of the train data and get the explained variance
        exvar = pca_obj.find_principal_components(xtrain)
        print("Explained variance: {:.2f}%".format(exvar * 100))
        
        # Reduce the dimensionality of the data using PCA
        data_reduced = pca_obj.reduce_dimension(xtrain)
        xtrain = data_reduced
        
        # Reduce the dimensionality of the test using PCA
        data_reduced = pca_obj.reduce_dimension(xtest)
        xtest = data_reduced
